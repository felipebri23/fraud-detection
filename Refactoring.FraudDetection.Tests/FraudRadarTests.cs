...
namespace Payvision.CodeChallenge.Refactoring.FraudDetection.Tests
{
    ...
    public class FraudRadarTests
    {
        ...
        public void CheckFraud_TwoLines_SecondLineFraudulent()
        {
            ...
            result.First().Item1.Should().BeTrue("The first line is not fraudulent");
            result.First().Item2.Should().Be(2, "The first line is not fraudulent");
        }
        ...
        private static IEnumerable<Tuple<bool, int>> ExecuteTest(string filePath)
        {
            var orders = OrderObjectMother.CreateFromFile(filePath);
